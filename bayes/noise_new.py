...
class NoiseTemplate:
    ...
    def loglike_contribution(self, model_error, prms):
        """
...
class NormalNoise(NoiseTemplate):
    def loglike_contribution(self, model_error, prms):
        ...
        ll -= len(model_error) / 2 * math.log(2 * math.pi / prec)
        ll -= 0.5 * prec * np.sum(np.square(model_error))
        return ll
