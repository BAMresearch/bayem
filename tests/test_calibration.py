...
# Tests:-----------------------------------
# 1. Checking the size of sampled posterior
# 2. Checking the shape of predictive posterior
class Sensor:
...
class Test_Caliberation(unittest.TestCase):
    #def test_something(self):
    #    self.assertEqual(True, False)

    def test_cali(self):
        #self.assertEqual(sum([1, 2, 3]), 6, "Should be 6")
        s1, s2 = MySensor("S1", 0.2), MySensor("S2", 0.5)
        ...
        def generate_data(N_time_steps, noise_sd):
            ...
            model_response = fw(prm, [s1, s2], time_steps)
            sensor_data = {}
        ...
        No_samples =10
        posterior = infer.run(No_samples, kernel="NUTS")

        self.assertEqual(np.size(posterior), No_samples)
        ...
        self.assertEqual(np.shape(pred_pos),(np.size(posterior),np.size(new_input_forward['sensors']) * new_input_forward['time_steps']))
